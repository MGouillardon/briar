...
<template>
  ...
  <section class="grid grid-cols-1 lg:grid-cols-2 gap-2">
    <Box v-for="property in properties.data" :key="property.id" :class="{'border-dashed': property.deleted_at}">
      <div class="flex flex-col md:flex-row gap-2 md:items-center justify-between">
        <div :class="{'opacity-25': property.deleted_at}">
          <div class="xl:flex items-center gap-2">
            <Price :price="property.price" class="text-2xl font-medium" />
            <PropertySpace :property="property" />
          </div>
          <PropertyAddress :property="property" class="text-gray-500" />
        </div>
        <div class="flex items-center gap-1 text-gray-600 dark:text-gray-300">
          <Link class="btn-outline text-xs font-medium" :href="route('property.show', {property: property.id})">Preview</Link>
          <Link class="btn-outline text-xs font-medium" :href="route('realtor.property.edit', {property: property.id})">Edit</Link>
          <Link
            v-if="!property.deleted_at"
            class="btn-outline text-xs font-medium"
            :href="route('realtor.property.destroy', {property: property.id})"
            as="button"
            method="delete"
          >
            Delete
          ...
          <Link
            v-else
            class="btn-outline text-xs font-medium"
            :href="route('realtor.property.restore', {property: property.id})"
            as="button"
            method="put"
          >
            Restore
